...
//                                  "/home/hide/catkin_ws/src/CNN-SVO/rpg_svo/monodepth-cpp/model/model_city2kitti.pb");
	...
	try{
		//torch::jit::script::Module depth_encoder;
		//torch::jit::script::Module depth_decoder;
		//depth_encoder = torch::load("/home/usuario/Desktop/LibTorch/depth_example/weights_19/encoder.pt");
		depth_encoder = torch::jit::load("/home/usuario/Desktop/LibTorch/depth_example/weights_19/encoder.pt");
		depth_encoder.to(at::kCUDA);
		
		//depth_encoder.to(at::kCPU);
		depth_decoder = torch::jit::load("/home/usuario/Desktop/LibTorch/depth_example/weights_19/depth.pt");
		depth_decoder.to(at::kCUDA);
	}
...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh,ImageAndExposure* image) {
	...
	if (allFrameHistory.size() == 2){
		initializeFromInitializerCNN(fh,image);
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id )
{
	...
	if(!initialized)
	{
		...
		if(coarseInitializer->frameID<0)	// first frame set. fh is kept by coarseInitializer.
		{
			...
			coarseInitializer->setFirst(&Hcalib, fh, getDepthMap(fh,image));
			initialized=true;
		}
	...
	else	// do front-end operation.
	{
		...
		Vec4 tres = trackNewCoarse(fh,image);
		...
		lock.unlock();
		deliverTrackedFrame(fh, needToMakeKF,image);
		return;
	}
}
void FullSystem::deliverTrackedFrame(FrameHessian* fh, bool needKF,ImageAndExposure* image)
{
	...
	if(linearizeOperation)
	{
		...
		if(needKF) makeKeyFrame(fh,image);
	...
	else
	{
		...
		unmappedTrackedImages.push_back(image);
...
void FullSystem::mappingLoop()
{
	...
	while(runMapping)
	{
		...
		ImageAndExposure* image = unmappedTrackedImages.front();
		unmappedTrackedFrames.pop_front();
		unmappedTrackedImages.pop_front();
...
void FullSystem::makeKeyFrame( FrameHessian* fh,ImageAndExposure* image)
{
...
void FullSystem::initializeFromInitializerCNN(FrameHessian* newFrame,ImageAndExposure* image)
{
	boost::unique_lock<boost::mutex> lock(mapMutex);
	...
	cv::Mat depth = getDepthMap(firstFrame,image);
	...
	void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth,ImageAndExposure* image)
{
	...
	cv::Mat depth = getDepthMap(newFrame,image);
    for (IOWrap::Output3DWrapper *ow : outputWrapper)
...
cv::Mat FullSystem::getDepthMap(FrameHessian* fh,ImageAndExposure* image2){
	cv::Mat depth;
	cv::Mat depth2;
	cv::Mat cv_image2_;
	...
	std::cout<<"Depth prediction start"<<std::endl;
	...
	//cv::Mat color = cv::Mat(model_height, model_width, CV_32FC1,image2->image);
	//std::cout<<color.channels()<<std::endl;
	//cv::namedWindow("FULL", cv::WINDOW_AUTOSIZE);
  	//cv::imshow("FULL", color);
  	//cv::waitKey(0);
	...
	cv_image2_ = cv::Mat(model_height, model_width, CV_32FC3);	
	cv::Mat colored_img, resized_f1,resized_f2;	
	cv::cvtColor(image,colored_img, cv::COLOR_GRAY2BGR);

  	colored_img.convertTo(cv_image2_, CV_32FC3, 1/255.0f);
  	cv::flip(cv_image2_, cv_image2_, 1);
	std::cout<<"Depth prediction to tensor"<<std::endl;
	torch::Tensor input_tensor = torch::from_blob(
  	cv_image2_.data, {1, model_height,model_width, 3});
	...
	std::cout<<"Sub/Div"<<std::endl;
	std::cout<<"Permute"<<std::endl;
	// to GPU
	std::cout<<"Depth prediction encoder-decoder"<<std::endl;
	auto output_tensor_ = depth_encoder.forward({input_tensor}).toTensorList(); 	
	std::cout<<"depth encoder"<<std::endl;	
	auto output2_tensor_ = depth_decoder.forward({output_tensor_}).toGenericDict();
	std::cout<<"depth decoder"<<std::endl;
	torch::Tensor t = output2_tensor_.at("disp_0").toTensor().squeeze().detach().cpu();

	std::cout<<"Depth prediction resize"<<std::endl;         
	depth = cv::Mat(model_height, model_width, CV_32FC1,t.data_ptr<float>());
	//std::cout<<depth.channels()<<std::endl;
	//std::cout<<depth<<std::endl;
	std::cout<<"Depth prediction resize1"<<std::endl;  
	resized_f2 = cv::Mat(hG[0], wG[0],CV_32FC1);
	//cv::Mat resized_f2(hG[0], wG[0], CV_8UC1);
	std::cout<<"Depth prediction resize2"<<std::endl;  
	cv::Size scale2(hG[0], wG[0]);
	std::cout<<"Depth prediction resize3"<<std::endl;  
	
  	cv::resize(depth, resized_f2, scale2);
	std::cout<<"Depth prediction resize4"<<std::endl;  


	//depthPredictor->inference(image, depth);
	std::cout<<"Depth prediction printing"<<std::endl;         
	depth2 = 0.3128f / (resized_f2 + 0.00001f);
	//return depth predicted --- TODO
	std::cout<<"Depth prediction end"<<std::endl;
