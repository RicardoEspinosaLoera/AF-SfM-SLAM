...
private:
	...
	Vec4 trackNewCoarse(FrameHessian* fh,ImageAndExposure* image);
	void traceNewCoarse(FrameHessian* fh);
	void activatePoints();
	void activatePointsMT();
	void activatePointsOldFirst();
	void flagPointsForRemoval();
	void makeNewTraces(FrameHessian* newFrame, float* gtDepth,ImageAndExposure* image);
	void initializeFromInitializer(FrameHessian* newFrame);
	void flagFramesForMarginalization(FrameHessian* newFH);

	void initializeFromInitializerCNN(FrameHessian* newFrame,ImageAndExposure* image);
 ...
 */
	...
	void makeKeyFrame( FrameHessian* fh,ImageAndExposure* image);
	void makeNonKeyFrame( FrameHessian* fh);
	void deliverTrackedFrame(FrameHessian* fh, bool needKF,ImageAndExposure* image);
	void mappingLoop();
	...
	std::deque<ImageAndExposure*> unmappedTrackedImages;
	...
	cv::Mat getDepthMap(FrameHessian* fh,ImageAndExposure* image2);
